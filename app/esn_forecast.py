...
def esn_forecast(models, data, input_data, selected_columns, horizon, forecasting_strategy, hyperparameters, new_variable_dataset, s=2):
    if horizon==1 and len(selected_columns)==1: # 1-step univariate
        ...
        if forecasting_strategy == "Recursive":
            ...
            train, test = denoised_variable_dataset[0:-1], denoised_variable_dataset[-1]
            ...
            testPredict = esn_make_predictions(models, trainX[-1], horizon)
            ...
            def predict_with_dirrec_esn_models(models, testX, model_type='all'):
                ...
                for h in range(horizon):
                    ...
                    testX = np.append(testX, prediction)
            ...
            if new_variable_dataset is not None and len(new_variable_dataset) > 0:
                ...
                print("Didn't enter to the condition")
            ...
            # make predictions
            testPredict, predictions_elapsed_time = predict_with_dirrec_esn_models(models, trainX[-1], 'esn')
            testPredict = np.array(testPredict)
            ...
            # make predictions
            testPredict, predictions_elapsed_time = esn_predict_using_models(models, trainX[-1], s)
            testPredict = np.array(testPredict)
